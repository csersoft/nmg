...
namespace NMG.Core
{
  public class DataTypeMapper
  {
    ...
    private bool IsUnsigned(string[] typePart)
    {
      if (typePart == null || typePart.Length == 0) return false;

      for (int i = 0; i < typePart.Length; i++)
      {
        if (typePart[i].ToUpper() == "UNSIGNED") return true;
      }
    ...
    {
      string[] dataTypePart = dataType.ToUpperInvariant().Split(new char[] { ' ', '\t', '(', ')' });
      string type;

      if (dataTypePart == null || dataTypePart.Length == 0) type = dataType.ToUpperInvariant();
      else type = dataTypePart[0];
      ...
      switch (type)
      {
        ...
        case "LONG":
        case "BIGINT":
          return IsUnsigned(dataTypePart) ? typeof(ulong) : typeof(long);

        case "SMALLINT":
          return IsUnsigned(dataTypePart) ? typeof(UInt16) : typeof(Int16);

        case "TINYINT":
          return IsUnsigned(dataTypePart) ? typeof(byte) : typeof(char);

        case "INT":
        case "INTERVAL YEAR TO MONTH":
        case "BINARY_INTEGER":
        case "INTEGER":
          return IsUnsigned(dataTypePart) ? typeof(uint) : typeof(int);
